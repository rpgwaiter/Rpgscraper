...
def get_albums(soup):
    ...
    for fulllink in link_array_unf:
        ...
        if link.startswith('https://downloads.khinsider.com/game-soundtracks/album/'):
            ...
            the_list.append((link.encode("utf-8"), get_songs(link, album_name)))
    return the_list


# Gets a set of songs from an album link
def get_songs(album_link, album_name):
    soup_album_link = BeautifulSoup(urllib2.urlopen(album_link), 'html.parser')
    ...
    for link_album_unf in array_album_unf:
        ...
        if str(link2).encode('utf-8').startswith("/game-soundtracks/album/" + album_name):
            if verbose:
                print("{0} -- Adding to song set".format(link2))
            the_set.add(("https://downloads.khinsider.com/" + link2.encode("utf-8")))
    return the_set


# Takes a link to the song, finds the file and downloads it.
def download_song(song_link):
    soup_song_page = BeautifulSoup(urllib2.urlopen(song_link), 'html.parser')
    ...
    for link_song_full in link_array_song:
        ...
        if link_song.encode('utf-8').startswith("http://66.90.93.122/"):
            mp3_link = link_song.encode("utf-8")

    # This is just the name of the mp3, fixes url encoding like %20
    mp3_filename = urllib2.unquote(mp3_link.split("/")[-1])
    ...
    if not os.path.isfile(mp3_fullpath):
        try:
            ...
            req_stream = urllib2.Request(mp3_link)
            response = urllib2.urlopen(req_stream)
...
for what_system in array_systems:
    ...
    soup_system = BeautifulSoup(urllib2.urlopen("https://downloads.khinsider.com/game-soundtracks/" +
